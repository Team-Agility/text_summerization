...
import copy
import nltk
from meetingMinute.language_model import LanguageModel
from meetingMinute.data.meeting import meeting_lists
...
def main(ids = meeting_lists.ami_development_set \
    ...
    for system_name in system_name_list:
        ...
        with open('meetingMinute/results/' + system_name + '_params_MSC_' + development_or_test + '.csv', 'w') as output_file:
    ...
    for corpus_id in corpus_id_range:
        ...
        path_to_tagged_corpus = 'meetingMinute/data/community_tagged/meeting/new/' + dataset_id + '_' + str(corpus_id) + '/'
        ...
        for meeting_id in ids:
            tagged_meeting = []
            with open(f'dataset/{meeting_id}/abstractive_summary.json') as f:
                data = json.load(f)
